...
class ThingModel extends Model {
  ...
  constructor(description) {
    ...
    if (description.hasOwnProperty('properties')) {
      for (const propertyName in description.properties) {
        //  遍历 properties，根据propertyName 获取properties的真实值
  ...
  initWebsocket() {
    ...
    //  onEvent 接收来自server.js的消息
    // 匿名函数 ，参数event作为匿名函数的参数，将匿名函数赋值给onEvent
    const onEvent = (event) => {
      ...
      switch (message.messageType) {
        case 'propertyStatus':
          console.log('before onPropertyStatus')
  ...
  updateProperties() {
    ...
    getPropertiesPromise.then((properties) => {
      console.log('getPropertiesPromise')
